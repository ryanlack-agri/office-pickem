  ...
  return (
    <button
      ...
      className={classNames(
        "flex flex-1 items-center gap-2 rounded-xl border px-3 py-3 text-left transition",
        active
        ...
        isWinner === true && "border-turf-400/60",
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-semibold text-chalk">{name}</span>
        {score !== null && (
          <span className="text-xs tabular-nums text-chalk/60">{score} pts</span>
        )}
      </span>
      {active && <span className="text-turf-400">●</span>}
    </button>
  );
}
  ...
  return (
    <div className="card p-3">
      <div className="mb-2 flex items-center justify-between px-1">
        <span className="text-xs font-semibold text-chalk/50">
          {g.state === "post"
            ? "Final"
            : g.state === "in"
            ? `Live · ${g.shortDetail}`
        ...
        {!locked ? (
          <span className="text-xs font-semibold text-turf-400">Locks in {timeUntil(g.kickoff)}</span>
        ) : gotIt ? (
          <span className="text-xs font-bold uppercase text-turf-400">✓ You got it</span>
        ) : missed ? (
          <span className="text-xs font-bold uppercase text-red-400">✗ Missed</span>
        ) : (
